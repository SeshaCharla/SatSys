...
The paramters of the saturated system are a function of the rate-constant for SCR reaction and the maximum surface
...
adsorption cite concentration. The change in $\alpha_{sat}$ with aging is demonstrated using the test-cell experiments (Figure~\ref{fig::alpha_sat_ssd}, \ref{fig::alpha_sat_iod}).
\begin{figure}[ht]
        \centering
...
\end{figure}
%===
\par Thus, $\theta_{sat}$ capture the aging of the catalyst. Hence, the catalyst aging detection problem can be posed as
a hypothesis testing problem with null hypothesis $\lr{\mathcal{H}_0}$ corresponding to a degreened catalyst and
alternative hypothesis $\lr{\mathcal{H}_1}$ corresponding to an aged catalyst. Since, the distribution of the error is
assumed to be known (half-normal) for both hypotheses and differ by the unknown (but estimatible) parameter
...
\end{align}
Where, $\theta_{sat}^{dg}$ is the parameter estimates for the degreened catalyst under same input conditions i.e.,
temperature, flow-rate and urea-dosing rate ranges. The threshold $\beta$ is decided based on the experimental results.
For RMC and FTP data sets, the value of the test-statistic is tabulated in Tabel~\ref{tab::Tw_tst}.
...
by the catalyst during the tests. Based on the results, a reasonabley discerning threshold for Hot-FTP and RMC test
