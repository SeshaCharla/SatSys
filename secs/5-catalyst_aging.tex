...
\end{figure}
%===
When the $\eta_{sat}$ response is nromalized with respect to the inlet $NO_x$ concentration and flow-rate, we end up with a quantity that is only a function of themperature and represents the product of rate-constant and maximum surface coverage, i.e., from \ref{eqn::regression}:
\begin{align}
...
\end{align}
%===
This quantity, $\alpha_{sat}$ is estimatible and changes with aging of the catalyst based on the hypothesis of changing adsorption cite concentration. The change in $\alpha_{sat}$ with aging is demonstrated using the test-cell experiments.
\begin{figure}[H]
        \centering
