...
\end{align}
%===
This quantity, $\alpha_{sat}$ can be estimated and changes with aging of the catalyst based on the hypothesis of
changing adsorption site concentration. The change in $\alpha_{sat}$ with aging is demonstrated using the test-cell
experiments (Figures~\ref{fig::alpha_sat_ssd} and \ref{fig::alpha_sat_iod}).
\begin{figure}[ht]
        \centering
...
assumed to be known (half-normal) for both hypotheses and differs by the unknown (but can be estimated) parameter
