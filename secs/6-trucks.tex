...
\begin{inparaenum}
        ...
        \item removing the rows whose operating temperature range is beyond the range of $200-300 \,^0C$,
...
$\mathcal{H}_0$. The convergence of the MLE to normality depends on the number of saturated-mode samples
\cite{kay1993estimation}, and truck data has considerably fewer such samples ($N_{sat} \approx 120$-$150$) compared
to test-cell experiments ($N_{sat} \approx 200$-$500$), resulting in heavier tails in the null distribution of $T_w$.
This effect is compounded by the less reliable estimation of the error variance under road conditions and the
...
With a larger data set and known degreend paramters, the thresholds can be determined using statistically optimal
...
readily available signals such as urea dosing rate, engine rpm, and throttle position. When these indicate sustained low
catalyst demand, the system can determine a priori that the accumulated data are unsuitable for aging detection and
defer its decision rather than executing the hypothesis test on insufficient data. The detector therefore avoids
producing false alarms under non-catalyst-saturating driving conditions, as the Wald test~(\ref{eqn::wald_test}) is
simply not invoked. A consequence of this design is that the monitor's in-use performance ratio (IUPMR) is governed by
the frequency of saturating driving conditions, since the diagnostic can only complete when the necessary
condition~(\ref{eqn::Ns_min}) is met.
