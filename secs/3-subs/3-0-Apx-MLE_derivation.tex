...
\begin{align}
        \ln \lr{p(\pmb \varepsilon_{\eta};\theta_{sat})} &= \lrb{N \ln N - N} - \ln \sum_k \lr{\phi_{\sat}(k)^T \theta_{\sat} - \eta(k)}
        \notag\\
        & \phi_{\sat}(k)^T \theta_{\sat} \geq \eta(k) \quad \forall k \in \lrf{1, 2, \hdots, N}
\end{align}
...
\begin{align}
       ...
       %===
       =\argmin_{\theta_{sat}}  \lrf{\sum_{k} \phi_{sat}(k) \theta_{\sat}}, \notag\\
       & s.t \quad \phi_{sat}(k) \theta_{\sat} \geq \eta_{\sat} (k) \quad \forall \: k \in \lrf{1, 2, \hdots, N}
\end{align}
