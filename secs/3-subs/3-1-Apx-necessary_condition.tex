...
\citep{AVLCruise}) tuned to RMC (Ramped Mode Cycle) test on a test-cell. The RMC test is then simulated with $\pm 20\%$
...
\end{figure}
Figure-\ref{fig::necessary_cond_sim} shows the actual $NO_x$ reduction $(\eta)$ from the simulation under the three dosing conditions, while Figure-\ref{fig::necessary_cond_sat} shows the corresponding predicted saturated-model response $(\eta_{sat})$ using parameters estimated from each case.
