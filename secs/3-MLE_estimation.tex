...
\begin{align}
        \text{minimize} &\qquad \norm{W \lr{\Phi_{sat} \theta_{sat} - H}}^2      \label{eqn::lin_prog}\\
        ...
        & \: W\text{ is the weight matrix and $\pmb 1$ is a column vector of $1's$.} \notag
\end{align}
The matrix $W$ is a diagonal matrix of weights to reduce the imbalance in sampling as the experimental conditions
cannot be fully controlled. The experimental results indicate that the constraint on the parameter signs
(\ref{eqn::theta_cond}) is not active for the optimal values showing that the model structure is consistent with data.
...
Physically, the assumption of half-normal distribution for $\varepsilon_\eta$ implies that the catalyst is saturated for
...
verified experimentally using goodness of fit for half-normal distribution demonstrated by the p-values \cite{kojadinovic2012goodness} in section-\ref{sec::epsilon_dist}.
