...
(\ref{eqn::eta_y_bound}) still holds. Writing interns of regression vector and introducing the constraints on the parameters \ref{eqn::theta_cond}, we have the linear programming problem for estimating the parameters of the catalyst:
\begin{align}
        \text{minimize} &\qquad \pmb 1^T \lrb{\pmb \Phi_{sat}^T \pmb \theta_{sat}}\\
        \text{subject to} &\qquad \pmb \Phi_{sat}^T \pmb \theta_{sat} \succeq \pmb H,
                           \qquad \bm{-1 & 0 & 0\\
                                      0  & 0 & 0 \\
