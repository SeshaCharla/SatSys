...
\end{align}
%===
The above approximate models for temperature introduces variation in the parameter estimates based on range and skewness in the sampling of the temperature. This can be quantified and accounted for using the sufficient statistics that are only the functions of temperature (Appendix-\ref{sec::T_lims}). Incorporating equations (\ref{eqn::eta}), (\ref{eqn::res_time}) and (\ref{eqn::gamma}) into the $NO_x$ process dynamic model (\ref{eqn::nox_mdl}) and writing in regression form, we get
\begin{align}
