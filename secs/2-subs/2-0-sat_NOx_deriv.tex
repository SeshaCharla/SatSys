...
\end{align}
\begin{multline}
  \con{NO_x}^{out}(t+t_s) = \con{NO_x}^{in} \\
...
\end{align}
%===
Let, $\eta_F(k)$ be the $NO_x$ reduction efficiency scaled by the mass flow rate at time step $k$,
\begin{align}
    \eta_F (k+1) &= F(k) \lrb{\frac{\con{NO_x}^{in}(k) - \con{NO_x}^{out}(k+1)}{\con{NO_x}^{in}(k)} }
\end{align}
...
\begin{align}
    \eta_F (k+1) &=   \tau_0 \; k_{scr}(T(k)) \; \gamma (T(k)) \label{eqn::flow_scaled_nox_eff}
\end{align}
%===
For $NO_x$ process dynamics under catalyst saturation, $\gamma(k) = \Gamma$.
\begin{align}
    \implies \eta_F^{sat} (k+1) &=   \tau_0 \; k_{scr}(T(k)) \; \Gamma (T(k))
        \label{eqn::nox_mdl}
\end{align}
%===
