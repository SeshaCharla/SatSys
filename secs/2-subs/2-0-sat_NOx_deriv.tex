%% 1. The NO_x reduction dynamics are different when the catalyst is saturated as it is independent of the urea-dosing.
%====
...
Letting tailpipe $NO_x$ concentration be $x_1(k+1) = \con{NO_x}^{out}(t+t_s)$ and inlet $NO_x$ concentration be $u_1(k) = \con{NO_x}^{in}(t)$, we get,
...
\end{align}
%===
The unbounded dynamics of $\gamma$, $\gamma^{ub}(k)$ are a nonlinear function dependent on the concentration of adsorbed ammonia, urea dosing $\lr{u_{inj}}$, and the inlet $NO_x$ concentration from the previous time step.
\begin{align}
...
$NO_x$ process dynamics (\ref{eqn::nox_avg}). $\gamma(k)$ can be described using the following piecewise
function:
\begin{align}
    \gamma(k) &=
    \begin{cases}
...
\end{align}
Introducing the above definition and the residence time expression (\ref{eqn::res_time}) into the $NO_x$ process dynamics model (\ref{eqn::nox_avg}), we get,
\begin{align}
...
\end{align}
%===
The above equation~(\ref{eqn::nox_mdl}) shows that the tailpipe $NO_x$ concentration becomes independent of urea dosing under catalyst saturation. Polynomial approximations of Arrhenius temperature dependence of the rate constant, $k_{scr}$ and the concentration of viable voids, $\Gamma$ \citep{nova2014urea,ciardelli2004scr,joo2008study}, are used to get a linear-in-parameters model for the $NO_x$ process dynamics.
\begin{align}
...
in the samples. For numerical stability \citep{press2003numerical}, the given temperature range is mapped to $[-1,1]$ and
Chebyshev polynomial bases \citep{trefethen2019approximation} are used instead of standard polynomials. Incorporating
equations (\ref{eqn::res_time}) and (\ref{eqn::gamma}) into the $NO_x$ process dynamics model (\ref{eqn::nox_mdl}),
\begin{align}
